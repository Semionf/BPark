 ...
 * application. Allows subscribers to update their email, phone number, and car
 * number. This controller handles user interactions, performs validation, and
 * sends formatted messages to the server.
 */
public class UpdateProfileController {
	/**
	 * Default constructor for UpdateProfileController. Required for JavaFX
	...
	/**
	 * Singleton-static instance reference to this controller. Allows other classes
	 * in the application to access this controller and update the UI directly.
	 */
	 ...
	 * FXML file is loaded. It sets the static instance reference and sends a
	 * request to the server to fetch the current subscriber's data using the
	 * logged-in user ID.
	 */
	@FXML
	...
	/**
	 * Handles the "Update" button click. This method: Reads and trims user input
	 * from all fields. Uses prompt text values as fallback if fields are left
	 * empty. Ensures at least one field has content before sending a request. Sends
	 * the updated information to the server Displays a success or error message in
	 * {@code statusLabel}.
	 * 
	 */
	@FXML
	...
	/**
	 * Sets prompt text for all input fields. This is typically called by the
	 * response handler after retrieving the current subscriber details from the
	 * server.
	 *
	...
	/**
	 * Sets the text of the email field. Add public setters so the
	 * ClientMessageHandler can set fields
	 *
